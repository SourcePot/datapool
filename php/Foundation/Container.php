...
class Container{
	...
	public function jsCall($arr){
		...
		if (isset($_POST['function'])){
			if (strcmp($_POST['function'],'container')===0){
				...
				$jsAnswer['html']=$this->arr['Datapool\Foundation\Container']->container($selector['ElementId'],'selectedView',$selector,array());
			} else if (strcmp($_POST['function'],'setCanvasElementPosition')===0){
				$jsAnswer['arr']=$this->arr['Datapool\Foundation\DataExplorer']->setCanvasElementPosition($_POST['arr']);
			} else {
				
			}
		} else if (isset($_POST['loadImage'])){
			$jsAnswer=$this->arr['Datapool\Tools\MediaTools']->loadImage($_POST['loadImage']);
		}
	...
	public function container($key=FALSE,$function='',$selector=array(),$settings=array(),$wrapperSettings=array(),$containerId=FALSE,$isJScall=FALSE){
		...
		//$this->arr['Datapool\Tools\ArrTools']->arr2file($_SESSION['container store'][$containerId]);
		...
		$html.=$this->arr['Datapool\Tools\HTMLbuilder']->element($reloadBtnArr);
		// add wrappers
		...
		$html=$this->arr['Datapool\Tools\HTMLbuilder']->element($wrapperDiv);
		return $html;
	}
	...
	private function selectedView($arr){
		...
		if (empty($arr['selector']['Source'])){
			$arr['html'].=$this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'P','element-content'=>'Nothing to show here. Entry-source missing.'));
		} else {
			// get setting
			...
			$arr['setting']=$this->arr['Datapool\AdminApps\Settings']->getSetting(__CLASS__,__FUNCTION__,$setting,$arr['selector']['Source'],TRUE);
			// compile html
			$primaryKeyValue=$this->arr['Datapool\Foundation\Database']->getPrimaryKeyValue($arr['selector']);
			$key=$primaryKeyValue['primaryKey'];
			if (empty($arr['selector'][$key])){
				if (!empty($setting['Show entry list'])){$arr=$this->entryList($arr);}
			} else {
				$arr['selector']=$this->arr['Datapool\Foundation\Database']->entryByKey($arr['selector']);
				if (empty($arr['selector'])){
					$arr['html']=$this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'P','element-content'=>'Nothing to show here. Entry does not exist.'));
				} else {
					if ($this->arr['Datapool\Tools\CSVtools']->isCSV($arr['selector'])){$arr=$this->arr['Datapool\Tools\CSVtools']->csvEditor($arr);}
					$arr=$this->arr['Datapool\Tools\MediaTools']->presentEntry($arr);
					if (!empty($arr['settingNeedsUpdate'])){
						$this->arr['Datapool\AdminApps\Settings']->setSetting(__CLASS__,__FUNCTION__,$arr['setting'],$arr['selector']['Source'],TRUE);
					}
					if (!empty($arr['setting']['Show comments editor'])){$arr=$this->comments($arr);}
					if (empty($arr['setting']['Show tools'])){$arr=$this->tools($arr);}
					if (!empty($arr['setting']['Show map'])){$arr=$this->arr['Datapool\Tools\GeoTools']->getMapHtml($arr,FALSE);}
				}
			}
	...
	public function entryEditor($arr,$isDebugging=FALSE){
		...
		if (!isset($arr['html'])){$arr['html']='';}
		$definition=$this->arr['Datapool\Foundation\Definitions']->getDefinition($arr['selector']);
		$tableInfo=$this->arr['Datapool\Foundation\Database']->getDbInfo($arr['selector']['Source']);
		$flatDefinition=$this->arr['Datapool\Tools\ArrTools']->arr2flat($definition);
		$entryCanWrite=!empty($this->arr['Datapool\Foundation\Access']->access($arr['selector'],'Write'));
		if (empty($arr['selector'])){
			$arr['html'].=$this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'p','element-content'=>'No entry found with the selector provided'));
		} else {
			...
			if (!empty($formData['cmd'])){
				if (isset($formData['cmd']['stepIn'])){
					...
					$arr['selector']=$this->arr['Datapool\Tools\ArrTools']->arrDeleteKeyByFlatKey($arr['selector'],$formData['cmd']['deleteKey']);
					$this->arr['Datapool\Foundation\Database']->updateEntry($arr['selector']);
				} else if (isset($formData['cmd']['addValue'])){
					$flatKey=$formData['cmd']['addValue'].$S.$formData['val']['newKey'];
					$arr['selector']=$this->arr['Datapool\Tools\ArrTools']->arrUpdateKeyByFlatKey($arr['selector'],$flatKey,'Enter new value here ...');
					$this->arr['Datapool\Foundation\Database']->updateEntry($arr['selector']);
				} else if (isset($formData['cmd']['addArr'])){
					$flatKey=$formData['cmd']['addArr'].$S.$formData['val']['newKey'].$S.'...';
					$arr['selector']=$this->arr['Datapool\Tools\ArrTools']->arrUpdateKeyByFlatKey($arr['selector'],$flatKey,'to be deleted');
					$this->arr['Datapool\Foundation\Database']->updateEntry($arr['selector']);
				} else if (isset($formData['cmd']['save']) || isset($formData['cmd']['reloadBtnArr'])){
					$arr['selector']=$this->arr['Datapool\Tools\ArrTools']->arrMerge($arr['selector'],$formData['val']);
					$this->arr['Datapool\Foundation\Database']->updateEntry($arr['selector']);
				} else {
			...
			while($key=array_pop($selectorKeyComps)){
				...
				$navHtml=$this->arr['Datapool\Tools\HTMLbuilder']->element($element).$navHtml;
			}
			// create table matrix
			$btnsHtml='';
			if ($entryCanWrite){
				$btnsHtml.=$this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'button','element-content'=>'&veeeq;','keep-element-content'=>TRUE,'key'=>array('save'),'value'=>'save','title'=>'Save','callingClass'=>$arr['callingClass'],'callingFunction'=>$arr['callingFunction']));
			}
			$btnArr=$arr['selector'];
			$btnArr['cmd']='download';
			$btnsHtml.=$this->arr['Datapool\Tools\HTMLbuilder']->btn($btnArr);
			$matrix=array('Nav'=>array('value'=>$navHtml,'cmd'=>$btnsHtml));
			if (!isset($settings['selectorKey'])){$settings['selectorKey']='';}
			$flatEntry=$this->arr['Datapool\Tools\ArrTools']->arr2flat($arr['selector']);
			foreach($flatEntry as $flatKey=>$value){
				...
				if (count($subFlatKeyComps)>1){
					...
					$valueHtml=$this->arr['Datapool\Tools\HTMLbuilder']->element($element);
				} else {
					// non-array value
					$element=$this->arr['Datapool\Foundation\Definitions']->selectorKey2element($arr['selector'],$flatKey,$value,$arr['callingClass'],$arr['callingFunction']);
					if (is_array($element)){
				...
				if (count($flatKeyComps)>1 && $level>0 && $entryCanWrite){
					...
					$cmdHtml=$this->arr['Datapool\Tools\HTMLbuilder']->element($element);
				}
			...
			$arr['html'].=$this->arr['Datapool\Tools\HTMLbuilder']->table(array('matrix'=>$matrix,'hideHeader'=>TRUE,'hideKeys'=>FALSE,'keep-element-content'=>TRUE,'caption'=>$arr['selector']['Name']));
		}
		if ($isDebugging){
	...
	private function entryList($arr,$isDebugging=FALSE){
		...
		if (!empty($formData['cmd'])){
			...
			$settings=$this->arr['Datapool\Tools\ArrTools']->arrMerge($settings,$formData['val']);
			// command processing
		...
		$addColoumnBtn=$this->arr['Datapool\Tools\HTMLbuilder']->element($element);
		$settings['columns'][]=array('Column'=>$addColoumnBtn,'Filter'=>FALSE);
		// get selector
		$filterSkipped=FALSE;
		$selector=$this->selectorFromSetting($arr['selector'],$settings,FALSE);
		$rowCount=$this->arr['Datapool\Foundation\Database']->getRowCount($selector,$settings['isSystemCall']);
		if (empty($rowCount)){
		...
		if (!empty($rowCount)){
			...
			$matrix=array();
			$columnOptions=array();
			foreach($this->arr['Datapool\Foundation\Database']->entryIterator($selector,$settings['isSystemCall'],'Read',$settings['orderBy'],$settings['isAsc'],$settings['limit'],$settings['offset']) as $entry){
				...
				if (empty($columnOptions)){
					...
					foreach($flatEntry as $flatColumnKey=>$value){
						$columnOptions[$flatColumnKey]=$this->arr['Datapool\Tools\ArrTools']->flatKey2label($flatColumnKey);
					}
				}
				foreach($settings['columns'] as $columnIndex=>$cntrArr){
					...
					if ($cntrArr['Filter']===FALSE){
						...
						$btnArr=$entry;
						$btnArr['cmd']='select';
						$matrix[$rowIndex][$columnIndex]=$this->arr['Datapool\Tools\HTMLbuilder']->btn($btnArr);
						$btnArr['cmd']='download';
						$matrix[$rowIndex][$columnIndex].=$this->arr['Datapool\Tools\HTMLbuilder']->btn($btnArr);
						$btnArr['cmd']='delete';
						$matrix[$rowIndex][$columnIndex].=$this->arr['Datapool\Tools\HTMLbuilder']->btn($btnArr);
						...
						$filterTextField=$this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'input','type'=>'text','style'=>$style,'value'=>$cntrArr['Filter'],'key'=>array('columns',$columnIndex,'Filter'),'callingClass'=>$arr['callingClass'],'callingFunction'=>$arr['callingFunction']));
						// order by buttons
						...
						$matrix[$filterKey][$columnIndex].=$this->arr['Datapool\Tools\HTMLbuilder']->element($element);
						...
						$matrix['Columns'][$columnIndex]=$this->arr['Datapool\Tools\HTMLbuilder']->select(array('options'=>$columnOptions,'value'=>$cntrArr['Column'],'keep-element-content'=>TRUE,'key'=>array('columns',$columnIndex,'Column'),'style'=>array(),'callingClass'=>$arr['callingClass'],'callingFunction'=>$arr['callingFunction']));
						if ($columnIndex>0){
							...
							$matrix['Columns'][$columnIndex].=$this->arr['Datapool\Tools\HTMLbuilder']->element($element);
						}
						// table rows
						if (strcmp($cntrArr['Column'],'preview')===0){
							$mediaArr=$this->arr['Datapool\Tools\MediaTools']->getPreview(array('selector'=>$entry,'style'=>array('width'=>'100%','max-height'=>100,'max-height'=>100)));
			...
			foreach($settings['columns'] as $columnIndex=>$cntrArr){
				if ($cntrArr['Filter']===FALSE){
					...
					$matrix['Limit<br/>offset'][$columnIndex]=$this->arr['Datapool\Tools\HTMLbuilder']->select(array('options'=>$otions,'key'=>array('limit'),'value'=>$settings['limit'],'title'=>'rows to show','callingClass'=>$arr['callingClass'],'callingFunction'=>$arr['callingFunction']));
					if ($rowCount>intval($settings['limit'])){
						...
						$matrix['Limit<br/>offset'][$columnIndex].=$this->arr['Datapool\Tools\HTMLbuilder']->element($element);	
					}
				} else {
					$matrix['Limit<br/>offset'][$columnIndex]='';
				}
			...
			$caption.=' ('.$rowCount.')';
			$arr['html'].=$this->arr['Datapool\Tools\HTMLbuilder']->table(array('matrix'=>$matrix,'hideHeader'=>TRUE,'hideKeys'=>FALSE,'keep-element-content'=>TRUE,'caption'=>$caption));
		}
		if ($isDebugging){
			$debugArr['arr out']=$arr;
			$debugArr['settings out']=$settings;
	...
	public function comments($arr){
		...
		foreach($Comments as $creationTimestamp=>$comment){
			$footer=$this->arr['Datapool\Tools\StrTools']->getDateTime();
			$footer.=' '.$this->arr['Datapool\Foundation\User']->userAbtract($comment['Author'],2);
			$commentHtml=$this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'p','element-content'=>$comment['Comment'],'class'=>'comment'));
			$commentHtml.=$this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'p','element-content'=>$footer,'class'=>'comment-footer'));
			$commentsHtml.=$this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'div','element-content'=>$commentHtml,'keep-element-content'=>TRUE,'class'=>'comment'));
		}
		$targetId=$arr['callingFunction'].'-textarea';
		$newComment='';
		if ($this->arr['Datapool\Foundation\Access']->access($arr['selector'],'Write')){
			$newComment.=$this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'h3','element-content'=>'New comment','style'=>array('float'=>'left','clear'=>'both','margin'=>'30px 0 5px 5px')));
			$newComment.=$this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'textarea','element-content'=>'...','key'=>array('comment'),'id'=>$targetId,'style'=>array('float'=>'left','clear'=>'both','margin'=>'5px','font-size'=>'1.5em'),'callingClass'=>$arr['callingClass'],'callingFunction'=>$arr['callingFunction']));
			$newComment.=$this->arr['Datapool\Foundation\Container']->container('Emojis for '.$arr['callingFunction'],'generic',$arr['selector'],array('method'=>'emojis','classWithNamespace'=>'Datapool\Tools\HTMLbuilder','target'=>$targetId));
			$newComment.=$this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'button','element-content'=>'Add','key'=>array('Add comment'),'value'=>time(),'style'=>array('float'=>'left','clear'=>'both','margin'=>'5px'),'callingClass'=>$arr['callingClass'],'callingFunction'=>$arr['callingFunction']));
			$newComment=$this->arr['Datapool\Tools\HTMLbuilder']->app(array('html'=>$newComment,'icon'=>'&#9998;','style'=>array('clear'=>'both')));
		}
	...
	public function tools($arr){
		...
		$btn['cmd']='download';
		$html.=$this->arr['Datapool\Tools\HTMLbuilder']->btn($btn);
		$btn['cmd']='delete';
		$html.=$this->arr['Datapool\Tools\HTMLbuilder']->btn($btn);
		$arr['html'].=$this->arr['Datapool\Tools\HTMLbuilder']->app(array('html'=>$html,'icon'=>'...'));
		return $arr;
	}
