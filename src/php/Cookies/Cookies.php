...
class Cookies implements \SourcePot\Datapool\Interfaces\App{
    ...
    public function run(array|bool $arr=TRUE):array
    {
        if ($arr===TRUE){
            ...
            $arr['toReplace']['{{content}}']=$this->oc['SourcePot\Datapool\Foundation\Container']->container('Cookies form','generic',['Source'=>$this->getEntryTable()],['method'=>'cookieForm','classWithNamespace'=>__CLASS__,],['style'=>['border'=>'none']]);
            return $arr;
        }
    }    
    ...
    public function cookieForm(array $arr):array
    {
        ...
        foreach($values as $name=>$value){
            $btn=['tag'=>'input','type'=>'submit','key'=>[$name,(boolval($value)?0:1)],'value'=>(boolval($value)?'TRUE':'FALSE'),'style'=>['line-height'=>'2rem','font-weight'=>'bold'],'class'=>(boolval($value)?'status-on':'status-off'),'callingClass'=>__CLASS__,'callingFunction'=>__FUNCTION__,'disabled'=>self::COOKIES[$name]['disabled']];
