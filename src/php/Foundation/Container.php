            ...
            foreach($this->oc['SourcePot\Datapool\Foundation\Database']->entryIterator($selector,$settings['isSystemCall'],'Read',$settings['orderBy'],$settings['isAsc'],$settings['limit'],$settings['offset'],[],TRUE,FALSE) as $entry){
                ...
                foreach($settings['columns'] as $columnIndex=>$cntrArr){
                    ...
                    if ($cntrArr['Filter']===FALSE){
                        ...
                        // fine filter rows
                        if (stripos(strval($flatEntry[$cntrArr['Column']]??''),strval($cntrArr['Filter']?:$flatEntry[$cntrArr['Column']]??''))===FALSE){
    ...
    public function comments(array $arr):array
    {
        ...
        if (isset($arr['selector']['Write'])){
            if ($this->oc['SourcePot\Datapool\Foundation\Access']->access($arr['selector'],'Write')){
                ...
                $newComment.=$this->oc['SourcePot\Datapool\Foundation\Element']->element(['tag'=>'textarea','element-content'=>$formData['val']['comment']??'','placeholder'=>'e.g. My new comment','key'=>['comment'],'id'=>$textId,'style'=>['float'=>'left','clear'=>'both','width'=>'95%','font-size'=>'1.2rem'],'callingClass'=>$arr['callingClass'],'callingFunction'=>$targetId,'excontainer'=>TRUE]);
                ...
                $appArr=['html'=>$newComment,'icon'=>'&#9871;','title'=>'Add comment','style'=>['clear'=>'both'],'open'=>!empty($formData['val']['comment'])];
