...
class Chat implements \SourcePot\Datapool\Interfaces\HomeApp{
    ...
    public function newEntryHtml(array $arr):array
    {
        ...
        $matrix=[];
        // New chat entry
        $matrix['Msg']['value']=$this->oc['SourcePot\Datapool\Tools\HTMLbuilder']->element(['tag'=>'textarea','element-content'=>$formData['val']['new']??'','placeholder'=>'e.g. This was a great day 😁','keep-element-content'=>TRUE,'style'=>['width'=>'90vw'],'id'=>$chatEntry['EntryId'],'key'=>['new'],'callingClass'=>$arr['callingClass'],'callingFunction'=>$arr['callingFunction'],'excontainer'=>TRUE]);
        ...
        $arr['html'].=$this->oc['SourcePot\Datapool\Tools\HTMLbuilder']->table(['matrix'=>$matrix,'hideHeader'=>TRUE,'hideKeys'=>TRUE,'keep-element-content'=>TRUE,'caption'=>'']);
        return $arr;
    }
