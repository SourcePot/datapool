...
class Explorer{
    ...
    private function accessInfo(string $callingClass,array $stateKeys):array
    {
        ...
        $matrix=[];
        $matrix['Read access']=['Your rols'=>$userRols,'Entry access for'=>$readRols,'Access granted'=>(empty($readAccess)?'FALSE':$readAccess)];
        $matrix['Write access']=['Your rols'=>$userRols,'Entry access for'=>$writeRols,'Access granted'=>(empty($writeAccess)?'FALSE':$writeAccess)];
