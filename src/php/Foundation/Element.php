...
class Element{
    ...
    const SECIAL_ATTR=[
        'function'=>FALSE,'method'=>FALSE,'target'=>FALSE,'trigger-id'=>FALSE,'container-id'=>FALSE,'canvas-element-id'=>FALSE,'excontainer'=>FALSE,'cell'=>FALSE,
    ...
    public function addclass2simpleTags(string $html,array $tags,string $styleClass):string
    {
        foreach($tags as $tag){
            $html=strtr($html,['<'.$tag.'>'=>'<'.$tag.' class="'.$styleClass.'">']);
        }
        return $html;
    }
