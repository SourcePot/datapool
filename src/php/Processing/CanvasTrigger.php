...
class CanvasTrigger{
	...
	private function canvasTriggerRules($callingElement){
		...
		$contentStructure=array('Trigger name'=>array('htmlBuilderMethod'=>'element','tag'=>'input','type'=>'text','value'=>$triggerInitName,'excontainer'=>TRUE),
							    ...
							    'Type'=>array('htmlBuilderMethod'=>'select','excontainer'=>TRUE,'keep-element-content'=>TRUE,'value'=>key($this->typeOptions),'options'=>$this->typeOptions),
								'Slope'=>array('htmlBuilderMethod'=>'select','excontainer'=>TRUE,'keep-element-content'=>TRUE,'value'=>key($this->slopeOptions),'options'=>$this->slopeOptions),
								);
		...
		$arr['caption']='Define trigger based on canvas elements';
	...
	public function getTrigger(){
		...
		foreach($this->arr['SourcePot\Datapool\Foundation\Database']->entryIterator($triggerEntriesSelector,TRUE,'Read') as $entryId=>$entry){
			foreach($entry['Content']['trigger'] as $triggerId=>$trigger){
				...
				$source=$this->arr['SourcePot\Datapool\Root']->class2source($entry['Folder']);
				if (isset($source)){
					$optionLabel=$source;
				} else {
