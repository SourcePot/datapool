...
class MediaTools{
	...
	public function getPreview($arr){
		...
		$file=$this->arr['Datapool\Tools\FileTools']->selector2file($arr['selector']);
		if (is_file($file)){
			if (!isset($arr['selector']['Params']['File']['MIME-Type'])){
				...
				$id=md5('wrapper'.$arr['selector']['Source'].$arr['selector']['ElementId']);
				$imageArr=array('tag'=>'div','element-content'=>$imageHtml,'keep-element-content'=>TRUE,'class'=>'preview','id'=>$id,'source'=>$arr['selector']['Source'],'element-id'=>$arr['selector']['ElementId']);
				$imageArr['style']=array('cursor'=>'pointer','padding'=>'3px');
				$arr['html'].=$this->arr['Datapool\Tools\HTMLbuilder']->element($imageArr);
				...
				$arr=$this->getPdf($arr);
			} else if ($this->arr['Datapool\Tools\CSVtools']->isCSV($arr['selector'])){
				$arr['html']='&plusb;';
			} else if (strpos($arr['selector']['Params']['File']['MIME-Type'],'text/')===0){
				$text=$this->arr['Datapool\Tools\FileTools']->file_get_contents_utf8($file);
				$arr=$this->addPreviewTextStyle($arr);
				$arr['tag']='p';
				$arr['element-content']=substr($text,0,200);
				$arr['html'].=$this->arr['Datapool\Tools\HTMLbuilder']->element($arr);
			} else {
	...
	public function getIcon($arr){
		...
		if (strpos($arr['selector']['Params']['File']['MIME-Type'],'image')===0){
			...
			$iconHtml=$this->arr['Datapool\Tools\HTMLbuilder']->element($iconArr);
		}
	...
	public function presentEntry($arr){
		if (empty($arr['selector'])){
			return $this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'p','element-content'=>__FUNCTION__.' called arr["selector"] being empty.'));
		} else if (!isset($arr['selector']['Content'])){
			return $this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'p','element-content'=>__FUNCTION__.' called, but arr["selector*]["Content"] is missing.'));	
		} else if (empty($arr['setting'])){
			return $this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'p','element-content'=>__FUNCTION__.' called with arr["setting"] being empty.'));	
		}
		if (!isset($arr['html'])){$arr['html']='';}
		...
		foreach($arrElements['elements'] as $presentationIndex=>$element){
			if (!empty($element['element'])){
				$arr['html'].=$this->arr['Datapool\Tools\HTMLbuilder']->element($element['element']);
			}
			if (isset($element['html'])){
				$arr['html'].=$element['html'];	
			}
		}
		if (!empty($arr['setting']['Show entry editor'])){$arr=$this->arr['Datapool\Foundation\Container']->entryEditor($arr);}
		if (empty($arr['returnHtmlOnly'])){return $arr;} else {return $arr['html'];}
	}
	...
	private function addElementFromKeySettingValue($arrElements,$key,$value){
		...
		if (isset($arrElements['elements'][$presentationIndex])){
			...
			$this->arr['Datapool\Foundation\Logging']->addLog(array('msg'=>$msg,'priority'=>44,'callingClass'=>__CLASS__,'callingFunction'=>__FUNCTION__));
		}
		...
		if (isset($arrElements['arr']['setting']['Key tags'][$key]['element'])){
			...
			if (strcmp($key,':::userAbstract')===0){
				...
				$widgetArr['selector']=array('Source'=>$this->arr['Datapool\Foundation\User']->getEntryTable(),'ElementId'=>$arrElements['arr']['selector']['Owner']);
				$arrElements['elements'][$presentationIndex]['html']=$this->arr['Datapool\Foundation\User']->userAbtract($widgetArr,2);
	...
	public function loadImage($arr){
		$image=$this->arr['Datapool\Foundation\Database']->entryByKey($arr);
		// get source file
		$imageFile=$this->arr['Datapool\Tools\FileTools']->selector2file($image);
		if (is_file($imageFile)){
			// get target file
			$absFile=$this->arr['Datapool\Tools\FileTools']->getTmpDir();
			$absFile.=$image['ElementId'].'_org.'.$image['Params']['File']['Extension'];
			//$absFile.=$this->arr['Datapool\Tools\StrTools']->getElementId().'.'.$image['Params']['File']['Extension'];
			$relFile=$this->arr['Datapool\Tools\FileTools']->abs2rel($absFile);
	...
	private function getVideo($arr){
		if (!isset($arr['html'])){$arr['html']='';}
		$video=$arr['selector'];
		$videoFile=$this->arr['Datapool\Tools\FileTools']->selector2file($video);
		if (is_file($videoFile)){
			// copy video to temp folder
			$absFile=$this->arr['Datapool\Tools\FileTools']->getTmpDir();
			$absFile.=$video['ElementId'].'.'.$video['Params']['File']['Extension'];
			copy($videoFile,$absFile);
			$videoArr=$arr;
			$videoArr['tag']='video';
			$videoArr['type']=$video['Params']['File']['MIME-Type'];
			$videoArr['src']=$this->arr['Datapool\Tools\FileTools']->abs2rel($absFile);
			$videoArr['element-content']=$arr['selector']['Name'];
			$videoArr['controls']=TRUE;
			$arr['html'].=$this->arr['Datapool\Tools\HTMLbuilder']->element($videoArr);
		}
		return $arr;
	}
	...
	private function getAudio($arr,$maxDim=FALSE){
		if (!isset($arr['html'])){$arr['html']='';}
		$audio=$arr['selector'];
		$audioFile=$this->arr['Datapool\Tools\FileTools']->selector2file($audio);
		if (is_file($audioFile)){
			// copy audio to temp folder
			$absFile=$this->arr['Datapool\Tools\FileTools']->getTmpDir();
			$absFile.=$audio['ElementId'].'.'.$audio['Params']['File']['Extension'];
			copy($audioFile,$absFile);
			$audioArr=$arr;
			$audioArr['tag']='audio';
			$audioArr['type']=$audio['Params']['File']['MIME-Type'];
			$audioArr['src']=$this->arr['Datapool\Tools\FileTools']->abs2rel($absFile);
			$audioArr['element-content']=$arr['selector']['Name'];
			$audioArr['controls']=TRUE;
			$arr['html'].=$this->arr['Datapool\Tools\HTMLbuilder']->element($audioArr);
		}
		return $arr;
	}
	...
	private function getPdf($arr){
		if (!isset($arr['html'])){$arr['html']='';}
		$sourceFile=$this->arr['Datapool\Tools\FileTools']->selector2file($arr['selector']);
		$tmpDir=$this->arr['Datapool\Tools\FileTools']->getTmpDir();
		$pdfFile=$tmpDir.$arr['selector']['Params']['File']['Name'];
		$this->arr['Datapool\Tools\FileTools']->tryCopy($sourceFile,$pdfFile);
		if (is_file($pdfFile)){
			$pdfArr=$arr;
			$pdfArr['tag']='embed';
			$pdfArr['src']=$this->arr['Datapool\Tools\FileTools']->abs2rel($pdfFile);
			$pdfArr['type']='application/pdf';
			$pdfArr['style']['float']='left';
			$pdfArr['style']['clear']='both';
			$pdfArr['style']['width']='98%';
			$pdfArr['style']['height']='500px';
			$arr['html'].=$this->arr['Datapool\Tools\HTMLbuilder']->element($pdfArr);
		} else {
			$arr['html'].=$this->arr['Datapool\Tools\HTMLbuilder']->element(array('tag'=>'div','element-content'=>'Sorry, file '.$arr['Params']['File']['Name'].' could not be copied into the presentation folder.'));
		}
	...
	private function getImage($arr){
		...
		if (stripos($arr['selector']['Params']['File']['MIME-Type'],'/svg')!==FALSE){
			return $this->arr['Datapool\Tools\FileTools']->file_get_contents_utf8($sourceFile);
		}
		//target file saved in the tmp directory
		$arr['targetFile']=$this->arr['Datapool\Tools\FileTools']->getTmpDir();
		$arr['targetFile'].=$arr['selector']['ElementId'].'.'.$arr['selector']['Params']['File']['Extension'];
		$arr['src']=$this->arr['Datapool\Tools\FileTools']->abs2rel($arr['targetFile']);
		...
		if (empty($tmpArr)){
			...
			return $this->arr['Datapool\Tools\HTMLbuilder']->element($arr);
		} else {
			// is an image
			...
			try{
				if (strpos($imgArr['fileType'],'/png')!==FALSE){
					...
					$string=$this->arr['Datapool\Tools\FileTools']->file_get_contents_utf8($imgArr['sourceFile']);
			...
			if (empty($arr['encodeBase64'])){
				...
				if (empty($arr['returnImgFileOnly'])){
					...
					$html=$this->arr['Datapool\Tools\HTMLbuilder']->element($imageTagArr);
